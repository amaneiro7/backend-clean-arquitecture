interface Props extends React.DetailedHTMLProps<React.ButtonHTMLAttributes<HTMLButtonElement>, HTMLButtonElement> {
  ...
  export function Button({ text, color, icon, className, handle, isDisabled = false, actionType, ...props }: Props) {
    return (
      <button      
        onClick={handle}      
        className={`flex gap-2 items-center w-full min-h-4 h-4 py-2 px-4 font-medium text-base align-middle text-center rounded-md cursor-pointer border border-solid transition-all duration-100 ease-in disabled:opacity-70 disabled:cursor-not-allowed ${className} ${ACTIONTYPE[actionType](color)}`}
        disabled={isDisabled}
        aria-label={`${text}`}
        title={`${text}`}
        {...props}
