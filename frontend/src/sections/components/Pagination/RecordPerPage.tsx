import { type LimitOptions, useDeviceContext } from "@/sections/Context/DeviceProvider"
...
export function RecordPerPage() {
    ...
    const { managePage:{ handleLimitChange }, limit } = useDeviceContext()
  return (
    <label htmlFor='records-per-page'>
      <select
        ...
        value={limit}
        onChange={(event) => {
            handleLimitChange(Number(event.target.value) as LimitOptions)
        }}
