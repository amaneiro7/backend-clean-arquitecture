const brandsData = require("../BrandData/brands")
const categoryData = require("../categoryData/categoryData")
const memoryRamType = require("../memoryRam/memoryRamType")
const processorSockets = require("../processor/processorSocket")

const modelComputer = [    
    { 
      id: "21969859-daa0-4916-a554-edff77e8f9ac", 
      name: "M70e", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id,       
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,       
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 775')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: "83c38f9b-a0ac-4350-9204-886ea8e4e967", 
      name: "M71e", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 1155')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3')[0].id,
      memoryRamSlotQuantity: 2,
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: true, 
      hasVGA: true
    },
    { 
      id: "9b43fe74-aa96-477e-8a25-cf3506742ccd", 
      name: "M72e", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 1155')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3')[0].id, 
      memoryRamSlotQuantity: 2, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: true, 
      hasVGA: true
    },
    { 
      id: "5da772a9-0e01-4689-9afb-717ccf68cf3f", 
      name: "M73z", 
      categoryId: categoryData.filter(category => category.name === 'All in One')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'LGA 1150')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: "4096012f-bed6-4975-a17c-6496c5573785", 
      name: "C260", 
      categoryId: categoryData.filter(category => category.name === 'All in One')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCBGA 1170')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3L')[0].id,
      memoryRamSlotQuantity: 1, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: true, 
      hasVGA: false
    },
    { 
      id: "1aa81dee-015b-4999-80f5-a5ecac793bca", 
      name: "M700", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'LGA 1151')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4')[0].id, 
      memoryRamSlotQuantity: 2, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: 'cb7c8589-4fbd-45c5-94d9-3d251273d295', 
      name: "M710S", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'LGA 1151')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4')[0].id, 
      memoryRamSlotQuantity: 2, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: '7003ef87-9563-4783-a758-16c471010243', 
      name: "M720S", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'LGA 1151')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4')[0].id, 
      memoryRamSlotQuantity: 2, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: 'ab48a708-cead-47c7-b974-9121c0e71265', 
      name: "M810Z", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'LGA 1151')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4')[0].id, 
      memoryRamSlotQuantity: 2, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: "ba260f98-0042-4be7-9d9d-1c6dca5010c3", 
      name: "M91p", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 1155')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3')[0].id, 
      memoryRamSlotQuantity: 4, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: "ec9e560a-318d-42a7-85cb-ad6fc08e57d9", 
      name: "M81", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 1155')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3')[0].id, 
      memoryRamSlotQuantity: 4, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: "c11d727c-f6b7-4ddf-a597-a6b1c1d06c1c", 
      name: "AD4", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 775')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR2')[0].id, 
      memoryRamSlotQuantity: 2, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },    
    { 
      id: "cdfd08c0-59d0-4bfc-a021-e6d53e1325fc", 
      name: "HP Compaq Elite 8300 SFF", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 1155')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3')[0].id, 
      memoryRamSlotQuantity: 4, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: '78e6fc44-7d45-47e6-9d78-aecc66ed74d4', 
      name: "HP Compaq Elite 8000 SFF", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 775')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3')[0].id, 
      memoryRamSlotQuantity: 4, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: '6c9eb83a-41df-4380-9a7e-2569bee4f03b', 
      name: "HP EliteDesk 800 G1 SFF", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'LGA 1150')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4')[0].id, 
      memoryRamSlotQuantity: 4, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false,
      hasVGA: true
    },
    { 
      id: '67d21f64-2fee-4818-849d-fe7d062287c2', 
      name: "HP EliteDesk 800 G4 SFF", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'LGA 1151')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3')[0].id, 
      memoryRamSlotQuantity: 4, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: true,
      hasVGA: true
    },
    { 
      id: "b67c290e-c186-4ab7-9fd3-92689e1bcbaa", 
      name: "HP Compaq Pro 4300 SFF PC", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 1155')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3')[0].id, 
      memoryRamSlotQuantity: 4, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: true, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: "4cb2e608-5f28-44af-af75-59cb2ca2fbab", 
      name: "HP Compaq Pro 6300 SFF", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 1155')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3')[0].id, 
      memoryRamSlotQuantity: 4, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: "9e376eb0-7559-41c1-82aa-36b6fa4cccf1", 
      name: "HP Compaq 6000 Pro SFF PC", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 775')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3')[0].id, 
      memoryRamSlotQuantity: 4, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: "ce51d092-332e-42ac-8b27-df7da5d3463a", 
      name: "HP Compaq 6200 Pro SFF PC", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 1155')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3')[0].id, 
      memoryRamSlotQuantity: 4, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: "8145c77f-a75e-4cfa-9634-a4eb17540cde", 
      name: "HP Compaq dc5700 Small Form Factor", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 775')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR2')[0].id, 
      memoryRamSlotQuantity: 4, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: "ff79cbfc-d1a1-4eb9-b2e3-dc7bc0037df9", 
      name: "HP Proliant ML110 Gen9", 
      categoryId: categoryData.filter(category => category.name === 'Servidores')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCLGA 2011')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4')[0].id,
      memoryRamSlotQuantity: 8, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: "bd0b6c7d-4d17-48cd-8b61-d4a167ad588f", 
      name: "HP Compaq dc5800 Small Form Factor", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 775')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR2')[0].id, 
      memoryRamSlotQuantity: 4, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: "15b7a063-4e99-4ec1-90ca-94a6e0edf000", 
      name: "HP Compaq dc5100 Small Form Factor", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id, 
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 775')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR2')[0].id,
      memoryRamSlotQuantity: 4, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: '868a0697-ecfa-471b-8705-31c298e6399a', 
      name: "HP 200 G1 MT", 
      categoryId: categoryData.filter(category => category.name === 'Computadoras')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Dell')[0].id, 
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCBGA 1170')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3L')[0].id,
      memoryRamSlotQuantity: 2,
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true
    },
    { 
      id: "bf10b490-f61d-4932-8bf4-05822213212d", 
      name: "HP EliteBook 8470p", 
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCPGA 988')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: false, 
      hasWifiAdapter: false, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true,
      batteryModel: '628668-001'
    },
    { 
      id: "ba8097b3-660f-4e09-a6fc-23c1e6332a90", 
      name: "HP EliteBook x360 1030 G3", 
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 775')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: true, 
      hasVGA: false,
      batteryModel: 'L02031-241'
    },
    { 
      id: "e52f9a4b-b708-4137-8d33-ca60497cfdbf", 
      name: "HP EliteBook x360 830 G6", 
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processor => processor.name === 'LGA 775')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: true, 
      hasVGA: false,
      batteryModel: 'L02031-241'
    },
    { 
      id: "a1a929ed-1be1-46fd-9da9-e7207b4f3dbc", 
      name: "HP Laptop 15t dy200", 
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCBGA 1449')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: true, 
      hasVGA: false,
      batteryModel: 'L11119-855'
    },
    { 
      id: "3fdb6958-f036-47e7-a583-89956e7f9d78", 
      name: "HP EliteBook 1040 G4", 
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCBGA 1440')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: true, 
      hasVGA: false,
      batteryModel: '805096-00512'
    },
    { 
      id: "206ecb35-1e43-47e2-85b4-f7b6405ab638", 
      name: "HP ProBook 4440s", 
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCPGA 988')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3L')[0].id,
      memoryRamSlotQuantity: 4, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true,
      batteryModel: '633802-001'
    },
    { 
      id: "eeb3420d-a6b8-4c0f-bcc8-048707ecc758", 
      name: "Latitude 5300", 
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Dell')[0].id, 
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCBGA 1528')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: true, 
      hasVGA: false,
      batteryModel: 'MXV9V'
    },
    { 
      id: "24a4ae7d-8570-45f3-b103-ea35a0f7c43c", 
      name: "Latitude 5400", 
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Dell')[0].id, 
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCBGA 1528')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: true, 
      hasVGA: false,
      batteryModel: '1V1XF'
    },    
    {
      id: 'eaf80960-440d-462b-8276-fac5227ba608',
      name: 'MacBook Pro Retina',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Apple')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCBGA 1168')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3')[0].id,
      memoryRamSlotQuantity: 1,
      hasBluetooth: true,
      hasWifiAdapter: true,
      hasDVI: false,
      hasHDMI: false,
      hasVGA: false,
      batteryModel: 'A1582'
    },
    {
      id: '666c3659-f97a-4f4b-a082-dc6cb87c5795',
      name: 'HP Pavilion dv4 Notebook PC',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCPGA 988')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true,
      batteryModel: 'L11119-855'
      
    },
    {
      id: '17212b01-df51-4a56-81ce-c02f67e3756b',
      name: 'HP Pavilion G4-1388la',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Hewlett-Packard')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCPGA 988')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true,
      batteryModel: '593553-001'
      
    },
    {
      id: '763223a6-bf6e-48bc-a5c7-4773cf7bdc2c',
      name: 'E49',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCPGA 988')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true,
      batteryModel: '710416-001'
      
    },
    {
      id: '6f20dc8d-da9d-4a71-860b-a2049314c0df',
      name: 'G480',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCPGA 988')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true,
      batteryModel: 'L11S6Y01'
      
    },
    {
      id: '8e7394ee-c130-4417-a398-a6c597c2e6e1',
      name: 'G50-70',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCBGA 1168')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true,
      batteryModel: 'L12M4E01'
      
    },
    {
      id: '67e1f755-1611-4f84-9478-08fd9bc35af3',
      name: 'L412',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCPGA 988')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true,
      batteryModel: '42T4795'
      
    },
    {
      id: '69d91018-3cd0-48d4-b33d-e8a0638cf693',
      name: 'SL400',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'Socket P')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR2L')[0].id, 
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true,
      batteryModel: '42T4704'      
    },
    {
      id: 'c558ac7b-0e11-4d4a-a0c7-1d39c630afd3',
      name: 'SL410',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'Socket P')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR2L')[0].id, 
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true,
      batteryModel: '42T4793'      
    },
    {
      id: '8279679f-afda-4102-bcd3-b34422ef3772',
      name: 'T61',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'Socket 478')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR2L')[0].id, 
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true,
      batteryModel: '42T5262' 
    },
    {
      id: '5f3103cb-bcba-4aeb-978c-50a2ad12c38b',
      name: 'V330-14IKB',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCBGA 1356')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true,
      batteryModel: 'L17M2PB5' 
    },
    {
      id: '208c2653-294a-4ed5-bf79-848dc312aac6',
      name: 'X230',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCBGA 1023')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: false, 
      hasVGA: true,
      batteryModel: '45N1025' 
    },
    {
      id: 'd983ef90-3a17-4dce-a0d1-6ba7c19d729e',
      name: 'X1 Carbon 7th',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'Socket P')[0].id, 
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR4L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: true, 
      hasVGA: false,
      batteryModel: '45N1025' 
    },
    {
      id: 'cbd39f5a-d5da-4f52-8a31-6a61c34cc088',
      name: 'X1 Carbon 5th',
      categoryId: categoryData.filter(category => category.name === 'Laptops')[0].id,
      brandId: brandsData.filter(brand => brand.name === 'Lenovo')[0].id,
      processorSocketId: processorSockets.filter(processorSocket => processorSocket.name === 'FCPGA 988')[0].id,
      memoryRamTypeId: memoryRamType.filter(memory => memory.name === 'DDR3L')[0].id,
      memoryRamSlotQuantity: 2, 
      hasBluetooth: true, 
      hasWifiAdapter: true, 
      hasDVI: false, 
      hasHDMI: true, 
      hasVGA: false,
      batteryModel: '45N1025' 
    },
  ]
  module.exports = modelComputer